...
var op = {
  ...
  $not: function(key, val, obop) {
    var sub = {};
    sub[key] = val;
    var where = obop.where(sub);
    return function(item) {
      return !where(item);
    };
  },
  $exist: function(key, val) {
    return function(item) {
      var exists = ('object' === typeof item) && ('undefined' !== typeof item[key]);
      return (exists === val);
    };
  },
...
exports.where = function(selector) {
  ...
  var err;
  Object.keys(complex).forEach(function(key) {
    if (err) return;
    var hash = complex[key];
    Object.keys(hash).forEach(function(type) {
      if (err) return;
      var val = hash[type];
      var gen = op[type];
      if (gen) {
        var f = gen(key, val, obop);
        if (f) array.push(f);
      } else {
        err = 'Unknown where operator: ' + type;
      }
    });
  });
